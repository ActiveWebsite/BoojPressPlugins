...
class wtd_plugin_update {
	...
	function __construct(){
		...
		$this->accessToken = '3cc43835ff58feb01636ef2a19abf745586082d5';
	...
	public function postInstall($true, $hook_extra, $result){
        ...
        $file_move = $wp_filesystem->move($result['destination'], $plugin_path);
    ...
    public function admin_head(){
        ...
        if(!empty($updates->response))
            foreach($updates->response as $slug => $plugin)
                if(substr_count($slug,'wtd-plugin.php') && $plugin->version != WTD_VERSION) {

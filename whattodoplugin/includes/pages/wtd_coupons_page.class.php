...
class wtd_coupons_page{
	
	private $disclaimer;
	public function __construct(){
	
		$this->disclaimer = 'Coupon Disclaimer: Print as many coupons as you want. Copyright© What to Do '.date('Y', time()).'. Unless stated coupon offers have no cash value, are subject to change, may have blackout dates, may contain errors & omissions, do not apply to sale items, cannot be used with other special offers or discounts & are one per person (no group orders).Check usage with merchant & present coupon before purchase.';
		...
		//Ajax calls
		add_action('wp_ajax_wtd_get_coupons',array(&$this,'get_coupons'));
		add_action('wp_ajax_nopriv_wtd_get_coupons',array(&$this,'get_coupons'));
	...
	public function big_menu($content){
			...
			jQuery(document).ready(function(){
				...
				<?php 
					if (!empty($wp_query->query['wtdc'])){
						?>
						cur_category = jQuery('[data-slug="<?php echo $wp_query->query['wtdc'];?>"]').attr('data-cat');
						if (!cur_category)
				...
				if (jQuery('[data-cat="'+cur_category+'"]').hasClass('parent_header_link')){
					...
					var parent = jQuery('[data-cat="'+cur_category+'"]').parent().parent();
					parent.slideToggle();
					var show = parent.attr('id');
					jQuery('[data-show="#'+show+'"]').addClass('active');
		...
		<?php 
			$exclude = array();
			if (!empty($wtd_plugin['coupons_options'][$res_id]))
				foreach ($wtd_plugin['coupons_options'][$res_id] as $cat_id => $cat){
					if (!empty($cat['parent']))
						$exclude[] = $cat_id;
				}
			$parent_cats = get_terms('wtd_category',array('parent' => 0,'exclude' => $exclude,'post_type' => array('wtd_coupon'),'cache_domain' => 'wtd_coupons_'.$res_id,'resorts' => array($res_id)));
		?>
		<div id="wtd-navigator">
			<div class="wtd_parent_menu">
				<span id="parent_0_header" class="wtd_parent_menu_item" data-show="#parent_0">
					<a data-show="#parent_0" data-cat="0" class="parent_header_link" href="<?php echo get_post_permalink($post->ID);?>">
						All
					</a>
				</span>
				<?php
				foreach($parent_cats as $parent_cat){ 
					?>
					<span id="parent_<?php echo $parent_cat->term_id;?>_header" class="wtd_parent_menu_item" data-show="#parent_<?php echo $parent_cat->term_id;?>">
						<a data-show="#parent_<?php echo $parent_cat->term_id;?>" data-slug="<?php echo $parent_cat->slug;?>" data-cat="<?php echo $parent_cat->term_id;?>" class="parent_header_link" href="<?php echo get_term_link($parent_cat,'wtd_category');?>"><?php
			...
			foreach($parent_cats as $parent_cat){
				?>
				<div id="parent_<?php echo $parent_cat->term_id;?>" class="wtd_menu"><?php
					$exclude = array();
					if (!empty($wtd_plugin['coupons_options'][$res_id][$parent_cat->term_id]['children']))
						foreach ($wtd_plugin['coupons_options'][$res_id][$parent_cat->term_id]['children'] as $cat_id => $cat){
								$exclude[] = $cat_id;
						}
					$categories = get_terms('wtd_category',array('parent' => $parent_cat->term_id,'exclude' => $exclude,'post_type' => array('wtd_coupon'),'cache_domain' => 'wtd_coupons_'.$res_id,'resorts' => array($res_id)));
					if(count($categories) > 0){
						for($i = 0; $i < count($categories); $i++){
							$cat = $categories[$i];?>
							<span class="wtd_menu_item">
								<a id="child_<?php echo $cat->term_id;?>" class="child_cat" data-slug="<?php echo $cat->slug;?>" data-cat="<?php echo $cat->term_id;?>" href="<?php echo get_term_link($cat,'wtd_category');?>"><?php
									echo $cat->name;?>
								</a>
							</span><?php
						}
	...
	private function matrix($content){
		...
		ob_start();
		?>
		<link rel="stylesheet" href="<?php echo WTD_PLUGIN_URL."assets/css/styles/wtd_menu.css";?>" media="screen"/>
		<?php
		$exclude = array();
	...
	private function left_side_menu($content){
		...
		ob_start();
		wp_enqueue_script( 'masonry', WTD_PLUGIN_URL.'assets/js/masonry.js', array(), '1.0.0', true );
		?>
		<link rel="stylesheet" href="<?php echo WTD_PLUGIN_URL."assets/css/styles/masonry.css";?>" media="screen"/>
		<link rel="stylesheet" href="<?php echo WTD_PLUGIN_URL."assets/css/styles/wtd_menu.css";?>" media="screen"/>
		<script>
			jQuery(window).ready(function(){
				//set the container that Masonry will be inside of in a var
				var container = document.querySelector('#wtd_parent_menu');
	...
	public function get_coupons(){
		...
		global $wtd_plugin;
		?>
		<div id="wtd_coupon_sc_container">
		<?php
			$paged = !empty($_POST['page']) ? $_POST['page'] : 1;
			$args = array(
				'posts_per_page' => 10,
				'paged' => $paged,
				'post_type' => 'wtd_coupon',
				'tax_query' => array(
					array(
						'taxonomy' => 'wtd_resort',
						'field' => 'id',
			...
			if (!empty($_POST['term_id']))
				...
				$exclude = array();
				if (!empty($wtd_plugin['couponse_options'][$_POST['res_id']]))
					foreach ($wtd_plugin['coupons_options'][$_POST['res_id']] as $term_id => $value){
			...
			if($query->found_posts):
				...
				foreach ($query->posts as $coupon):
					$vendor_metas = $wpdb->get_results(
						$wpdb->prepare(
							'SELECT pm.*,p.post_title as vendor FROM '.$wpdb->prefix.'postmeta pm
					...
					foreach ($vendor_metas as $vend_data){
						...
						$vendor = $vend_data->vendor;
					}
					$metas = $wpdb->get_results($wpdb->prepare('SELECT * FROM '.$wpdb->postmeta.' WHERE post_id = %s',$coupon->ID));
					foreach ($metas as $key => $meta){
						$name = $meta->meta_key;
						$$name = $meta->meta_value;
					}
					?>
					<div class="wtd_coupon_container">
						<div class="wtd_coupon_sc_coupon_list">
							<div class="wtd_coupon_sc_top_content clearfix">
								<div class="wtd_coupon_sc_imageArea">
									<img src="<?php echo wtd_get_image_url($coup_thm); ?>" alt="Vendor Image"/>
								</div>
								<div class="print">
									<a target="_blank" href="<?php echo get_home_url().'/?wtd_print_coupon='.$coupon->ID ?>">
										Print
									</a>
								</div>
								<div class="wtd_coupon_sc_centerContent">
									<p>
										<span><?php echo $vendor;?></span>
										<br/>
										<a href="<?php echo get_post_permalink($coupon->ID); ?>"><?php
											echo htmlspecialchars_decode($coupon->post_title);?>
											From <?php echo $vendor;?>
										</a>
									</p>
                                    <?php if (!empty($coupon_code)): ?>
                                        <p>
                                            Coupon code -
                                            <strong> &laquo; <?php echo $coupon_code; ?> &raquo; </strong>
                                        </p>
                                    <?php endif; ?>
									<p><?php
										echo htmlspecialchars_decode($coupon->post_content);?>
										<br/>
										<span class="expire">Expire <?php echo date('Y-m-d', strtotime($coup_end)); ?></span>
									</p>
								</div>
							</div>
							<p class="textItalic">
								Coupon Rules: Print as many coupons as you want. <?php
	...
	private function results($content){
		...
		<link rel="stylesheet" href="<?php echo WTD_PLUGIN_URL.'assets/css/wtd_coupons_page.css'; ?>" media="screen"/>
		<?php $masonry_item = '';?>
		<div id="wtd_coupon_sc_container">
            <?php
            $exclude = array();
			...
			if($query->found_posts):
					...
					<div class="wtd_coupon_container">
						<div class="wtd_coupon_sc_coupon_list">
							<div class="wtd_coupon_sc_top_content clearfix">
								...
								<div class="print">
									<a target="_blank" href="<?php echo get_home_url().'/?is_print=1&wtd_print_coupon='.$coupon->ID ?>">
										Print
									</a><br/>
									<a href="javascript:void(0);" class="wtd_mailer" data-mail="<?php echo $coupon->ID;?>">
										Send by e-mail
									</a>
								</div>
								<div class="wtd_coupon_sc_centerContent">
                                        ...
                                        foreach ($addresses as $address):?>
                                            <p>
                                                ...
                                                if (!empty($address['street']))
                                                    $location_display .= $address['street'];
				...
				No listings of this type are available.
            <?php endif;?>
            <script>
                jQuery('.wtd_mailer').click(function(e){
                    e.preventDefault();
                    var id = jQuery(this).attr('data-mail');
			...
			<div id="wtd_metabox_content" style="display:block;">
				...
				<div id="wtd_categories">
                    ...
                    <?php
                        ...
                        foreach ($wtd_excluded_categories as $x)
                            unset($cats[$x]);
...
$wtd_coupons_page = new wtd_coupons_page();

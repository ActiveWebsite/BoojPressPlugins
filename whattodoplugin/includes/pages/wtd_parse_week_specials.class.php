...
if (!class_exists('wtd_parse_week_specials')) {
    class wtd_parse_week_specials
    {
        ...
        public function __construct()
        {
            ...
            //Page Content
            if (!empty($wtd_plugin['week-specials-page'])) {
                add_action('the_content', array(&$this, 'page_content'), 99);
            }

            //Ajax calls
            add_action('wp_ajax_wtd_build_week_specials', array(&$this, 'build_week'));
            add_action('wp_ajax_nopriv_wtd_build_week_specials', array(&$this, 'build_week'));
        ...
        public function page_content($content)
        {
            ...
            if (!empty($wtd_pages['week_specials_pages']))
                if (in_array($post->ID, $wtd_pages['week_specials_pages'])) {
        ...
        public function content($content)
        {
            ...
            $resort = $wpdb->get_var(
                $wpdb->prepare(
                    'SELECT wtd_term_id FROM ' . $wpdb->prefix . 'wtd_meta WHERE term_id = %s AND meta_key = "res_id"',
                    $res_id
                )
            );
            ...
            $wtd_base_request['start'] = date('c');
            $wtd_base_request['end'] = date('c', time() + (7 * 24 * 60 * 60));
            ?>
            ...
            <script>
                ...
                var wtd_base_request = <?php echo json_encode($wtd_base_request);?>;
                    function wtd_parse_events_week() {
                        Parse.Cloud.run('specials_week_short', wtd_base_request, {
        ...
        public function build_week()
        {
            ...
            if (!empty($data)):
                ...
                foreach ($events as $key => $event):
                    ...
                    $vendor = $event->vendor;
                    $event_url = $post_url . $event->id . '/' . sanitize_title($event->name);
                    ?>
                    <div class="wtd_listing_container">
                        <div class="wtd_listing_sc_listing_list">
                            <div class="wtd_listing_sc_top_content clearfix">
                                <?php
                                $title = $event->name;
                                ?>
                                <p class="wtd_listing_title_bar"><a href="<?php echo $event_url; ?>"><?php echo $title; ?></a></p>

                                <div class="wtd_listing_sc_imageArea">
                                    <a href="<?php echo $event_url; ?>">
                                        <img src="<?php echo wtd_get_image_url($event->image); ?>" alt="<?php echo $title; ?>"/>
                                    </a>
                                </div>
                                <div class="wtd_listing_sc_centerContent">
                                    <ul>
                                        <li>
                                            ...
                                            <?php if (!empty($vendor)): ?>
                                                <p>Event Hosted by: <?php echo $vendor; ?></p>
                                            <?php endif;?>
                                            ...
                                            <?php if($event->startTime != "00:00:00" && $event->startTime != "23:59:59" && !empty($event->startTime)):?>
                                                <p class="wtd_week_date">
                                                    <?php
    ...
    $wtd_parse_week_specials = new wtd_parse_week_specials();
}
?>
    ...
    class wtd_parse_single_activity
    {
        ...
        public function activity_content($content){
            ...
            if ($post->post_type == 'wtd_activity') {
                ...
                $post_terms = wp_get_post_terms($post->ID, 'wtd_category');
                $res_cat_id = $wpdb->get_var('SELECT term_id FROM ' . $wpdb->prefix . 'wtd_meta WHERE meta_key = "cat_id" AND wtd_term_id = 6 LIMIT 1');
                $resort_id = $wpdb->get_var(
                    $wpdb->prepare(
                        'SELECT term_id FROM ' . $wpdb->prefix . 'wtd_meta WHERE meta_key = "res_id" AND wtd_term_id = %s LIMIT 1',
                        $res_id
                    )
                );
                $page_args = array(
                ...
                foreach ($post_terms as $post_term) {
                    if ($post_term->term_id == $res_cat_id || $post_term->parent == $res_cat_id) {
                        $dining = true;
                    ...
                    if (!in_array($post_term->term_id, $bookable_categories))
                        $booking = false;
                }
                if (!$dining)
                    $page_args['meta_query'][] = array(
                ...
                if ($page_query->found_posts):
                    $back_page = $page_query->posts[0];

                    $first = $term = $post_terms[0];
                    $back_url = get_post_permalink($back_page) . 'category/' . $first->slug;
                    if ($term->parent) {
                        $first = get_term($term->parent, 'wtd_category');
                        $second = $term;
                        $back_url = get_post_permalink($back_page) . 'category/' . $second->slug;
                    }
                    $first_url = get_post_permalink($back_page) . 'category/' . $first->slug;
                    if ($dining)
                        $first_url = get_post_permalink($back_page);
                    ?>
                    <div id="wtd_breadcrumb">
                        <a href="<?php echo get_post_permalink($back_page); ?>" class="wtd_pull_left"><?php echo get_the_title($back_page); ?></a>
                        <span class="wtd_bread_separator">&gt;</span>
                        <a id="parent_<?php echo $first->term_id; ?>_header" href="<?php echo $first_url; ?>" class="wtd_pull_left"><?php
                            echo $first->name;?>
                        </a>
                        <?php if (!empty($second)): ?>
                            <span class="wtd_bread_separator">&gt;</span>
                            <a id="parent_<?php echo $second->term_id; ?>_header" href="<?php echo get_post_permalink($back_page) . 'category/' . $second->slug; ?>" class="wtd_pull_left">
                                <?php
                                echo $second->name;
                                ?>
                            </a>
                ...
                foreach ($post_metas as $meta) {
                    $meta_name = $meta->meta_key;
                    $$meta_name = maybe_unserialize($meta->meta_value);
                }
                ?>

                <div id="wtd_listing_sc_container">
                    <?php
                    $vend_id = get_post_meta($post->ID, 'vend_id', true);
                    $vend_args = array(
                        'posts_per_page' => 1,
                    ...
                    <div class="wtd_single_listing_container">
                        <div class="wtd_listing_sc_listing_list">
                            <div class="wtd_listing_sc_top_content clearfix">
                                <div class="wtd_single_listing_sc_image" class="wtd_text_center">
                                    <img class="wtd_top_10" src="<?php echo $listing_thm; ?>" alt="<?php echo $post->post_title;?>"/>
                                </div>
                                <div class="wtd_single_listing_sc_left_content" id="wtd_parse_content">
                                    <?php
                                    if (empty($first)) {
                                        $first = $post_terms[0];
                                        if ($first->parent)
                                            $first = get_term($first->parent, 'wtd_category');
                                    }
                                    ?>
                                    <p class="wtd_text_bold"><?php echo $vendor->post_title; ?></p>
                                    <p><?php echo $post->post_content; ?></p>
                                    <?php if ($page_query->found_posts): ?>
                                        <p>
                                            <a href="<?php echo $back_url; ?>" id="back_link">
                                                Back to listings
                                            </a>
                                            <script>
                ...
                $base_request['list_id'] = $list_id;
                if ($dining)
                    $base_request['dining'] = 1;
                ?>
                <script>
                ...
                <script>
                    ...
                    var wtd_base_request = <?php echo json_encode($base_request);?>;
                        var wtd_post_id = <?php echo $post->ID;?>;

                        function wtd_parse_activity(count) {
                            if (count < 3) {
                                Parse.Cloud.run('get_activity', wtd_base_request, {
                                    ...
                                    error: function (error) {
                                        if (error.message.indexOf('Token') === -1) {
                                            <?php if ($dining):?>
                                            var trigger_url = 'http://admin1.whattodo.info/admin/support/scp/ajax/parse/data_handler.php?action=getDining&itemid=<?php echo $list_id;?>';
                                            <?php else: ?>
                                            var trigger_url = 'http://admin1.whattodo.info/admin/support/scp/ajax/parse/data_handler.php?action=getActivity&itemid=<?php echo $list_id;?>';
                                            <?php endif; ?>
                                            jQuery.ajax({
                                                url: trigger_url,
                                                success :function(){
                                                    count = count + 1;
                                                    wtd_parse_activity(count);
                                                }
                                            });
                                        }else{
        ...
        public function build_content(){
            ...
            ?>
            <?php if (!empty($videos)): ?>
                <?php $wtd_video_id = wtd_video_id_generator($videos[0]->imageUrl); ?>
            ...
            <?php endif; ?>
            <?php if (!empty($first)): ?>
                <?php if ($first->name != 'Restaurants'): ?>
                    <?php if (!empty($wtd_plugin['wtd_booking_button']) && $booking): ?>
                        <a href="<?php echo WTD_LEAD_BASE . $activity->list_id; ?>" target="_blank" class="wtd_booking_button">Book Now</a>
                    <?php endif; ?>
                <?php endif; ?>
            <?php endif; ?>
            <p class="wtd_text_bold"><?php echo $vendor->displayName; ?></p>
            <p><?php echo $activity->description; ?></p>

            <p class="wtd_text_bold">Contact Information</p>
            <?php if (!empty($addresses)):
                ...
                <?php endforeach; ?>
            <?php endif; ?>
            <?php if (!empty($activity->phone)): $listing_phone = $activity->phone; ?>
                <p>
                    <span
                        class="expireBig"><?php echo " (" . substr($listing_phone, 0, 3) . ") " . substr($listing_phone, 3, 3) . "-" . substr($listing_phone, 6); ?></span>
                </p>
            <?php endif; ?>
            <?php  if (!empty($activity->website)): $listing_web = $activity->website; ?>
                <p>
                    <span class="expireBig">
                        <a href="<?php echo (substr_count($listing_web, 'http://')) ? $listing_web : 'http://' . $listing_web; ?>" target="_blank">
                            <?php echo $listing_web; ?>
                        </a>
                    </span>
                </p>
            <?php endif;
                if (!empty($images)) {
                    ?>
                    <div id="wtd_act_gallery">
                        <div id="wtd_gallery_prevnext">
                            <a href="javascript:void(0);" id="wtd_gallery_prev">&lt; Previous</a> |
                            <a href="javascript:void(0);" id="wtd_gallery_next">Next &gt;</a>
                        </div>
                        <div id="wtd_gallery_big">
                            <?php
                            echo '<img src="' . wtd_get_image_url($images[0]->imageUrl) . '"/>';
                            ?>
                        </div>
                        <div id="wtd_gallery_nav">
                            <?php
                            foreach ($images as $key => $image) {
                                $class = ($key == 0) ? 'current_image' : '';
                                echo '<a href="javascript:void(0);" class="' . $class . '" data-img="' . wtd_get_image_url($image->imageUrl). '"><img src="' . wtd_get_image_url($image->thumbUrl) . '"/></a>';
                            }
                            ?>
                        </div>
                    </div>

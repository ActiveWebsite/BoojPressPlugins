...
class wtd_specials_page{
	...
	public function __construct(){
		...
		//Ajax calls
		add_action('wp_ajax_get_wtd_month_specials',array(&$this,'wtd_get_month'));
		add_action('wp_ajax_nopriv_get_wtd_month_specials',array(&$this,'wtd_get_month'));
	...
	public function page_content($content){
		...
		?>
			...
			<script>
                ...
                function calendar_repair(){
                    ...
                    if (jQuery('.day.type1').length > 0){
                        var height = jQuery('#dates img').first().height();
                        jQuery('#dates .day').height(1.1*height);
                    }
                    if (jQuery('.day.type2').length > 0){
                        var max_height = 0;
                        jQuery('#dates .day').each(function(){
                            if (jQuery(this).height() > max_height)
                                max_height = jQuery(this).height();
                        });
                        jQuery('#dates .day').height(max_height);
                    }
                    if(jQuery('#dates .small-events').length > 0){
                        var max_height = 0;
                        jQuery('#dates .small-events').each(function(){
                ...
                function modal_repair(sel){
                    ...
                    jQuery('.wtd_events_modal .contents a').height(1.4 * img_height);
                    var max_height = 0;
                    ...
                    var bottom = img_width/13;
                    jQuery('.wtd_events_modal .contents a span').each(function(){
                        var temp_height = parseInt(jQuery(this).height());
                        if (max_height - temp_height)
                            jQuery(this).css('bottom',bottom+'px');
                    });
                }

                function wtd_get_month(month, year, force){
                    jQuery.ajax({
                        type: 'POST',
                        dataType: 'JSON',
                        url: wtd_ajax_url,
                        data: {
                            action: 'get_wtd_month_specials',
                            month: month,
                            year: year,
                        ...
                        success: function(data){
                            ...
                            jQuery('#prev_month').attr('data-month',data.prev_month).html(data.prev_month_text).attr('data-year',data.prev_year);
                            jQuery('#next_month').attr('data-month',data.next_month).html(data.next_month_text).attr('data-year',data.next_year);
                            jQuery('#curent_month').html(data.curent_month);
                            calendar_repair();
                        }
                    });
                }
                ...
                jQuery(document).ready(function(){
                    //Check month
                    //~ if($.cookie('wtd_calendar_month')){
                    //~ get_month($.cookie('wtd_calendar_month'));
                    //~ $.removeCookie('wtd_calendar_month',false);
                    //~ }
                    ...
                    jQuery('.month_loader').on('click', function(event){
            ...
            ?>
			<div class="wtd_row" id="calendar_controler">
				<a href="javascript:void(0);" id="prev_month" class="get_month" data-month="" data-year="">&nbsp;</a>
				<span id="curent_month"><?php echo date('F Y');?></span>
				<a href="javascript:void(0);" id="next_month" class="get_month" data-month="<?php echo $next_month;?>" data-year="<?php echo $next_year;?>"><?php echo $next_month_name;?> &raquo;</a>
			</div>
			<div id="wtd_events">
				...
				<div id="dates">
					<div class="wtd_row"><?php
						...
						$test_timestamp = $first_timestamp;
						$before = '';
						if($current_day_week != "Sunday"){
							while(date('l', $test_timestamp) != "Sunday"){
                                                                $test_timestamp = $test_timestamp - $step;
							}
						}
						$temp_timestamp = $test_timestamp;
						$month = date('m',$temp_timestamp);
						$type = $wtd_plugin['specials_type'];
						$check_day = false;
                        $i = 0;
						while ($check_day != 'Sunday' || $month != $next_month){
							if($i % 7 == 0 && $i != 0)
							...
							$args = array(
								...
								'meta_query' => array(
									array(
										'key' => 'eve_date',
										'value' => date('Y-m-d', $temp_timestamp)
									)
								),
								'tax_query' => array(
									array(
							...
							if (!empty($wtd_plugin['excluded_locations'])){
								$args['tax_query'][] = array(
									'taxonomy' => 'wtd_location',
									'field' => 'id',
									'terms' => array($wtd_plugin['excluded_locations']),
									'operator' => 'NOT IN'
								);
							}
							if (!empty($wtd_plugin['specials_options'][$res_id])){
								$category = get_term_by('name','Event Calendar','wtd_category');
								$not_categories = array_keys($wtd_plugin['specials_options'][$res_id][$category->term_id]['children']);
								$args['tax_query'][] = array(
									'taxonomy' => 'wtd_category',
									'field' => 'id',
									'terms' => $not_categories,
									'operator' => 'NOT IN'
								);
							}
							$eve_query = new WP_Query($args);
							$results = $eve_query->posts;
							$modal = '';
							if (!$eve_query->found_posts)
                                $modal = 'nomodal';
							switch ($type):
								case 1:
									$images = '';
									if (!empty($results)){
										$rand_event = $results[array_rand($results)];
										$image = get_post_meta($rand_event->ID,'eve_thm',true);
										$images = '<img src="'.WTD_IMG_BASE.$image.'" class="main_thumb"/>';
									}
								...
								case 2: 
									...
									if (!empty($results)){
										for($k = 0; $k < 4; $k++)
											if (!empty($results[$k])){
												$images .= '<img src="'.WTD_IMG_BASE.get_post_meta($results[$k]->ID,'eve_thm',true).'"/>';
											}
									}
									$events_text = count($results).' &nbsp;&nbsp;Specials';
									break;
								case 3:
									...
									if (!empty($results)){
										...
										foreach($results as $key => $row){
											if($k == 3)
												break;
											$images .= '<a href="'.get_post_permalink($row->ID).'">&middot; '.$row->post_title.'</a>';
											$k++;
										}
										$images .= '</div>';
									}
							...
							<div data-i="<?php echo $i;?>" data-date="<?php echo $temp_timestamp;?>" class="day type<?php echo $type.' '.$not_month.' '.$modal;?>">
								<span class="current_date"><?php echo date('M jS', $temp_timestamp);?></span>
								...
								if (!$modal) {?>
									<div id="modal_<?php echo $i; ?>" class="wtd_events_modal">
										<div class="control">
											<h2><?php echo date('l, jS F', $temp_timestamp); ?></h2>
										...
										<div class="contents">
											<div class="wtd_row"><?php
												...
												foreach ($results as $row) {
													if ($j % 5 == 0 && $j != 0)
                                                        echo '</div><div class="wtd_row">';?>
													<a href="<?php echo get_post_permalink($row->ID); ?>"
													title="<?php echo $row->post_title; ?>">
													<img src="<?php echo WTD_IMG_BASE.get_post_meta($row->ID,'eve_thm',true); ?>"
													alt="<?php echo $row->post_title; ?>"/>
													<span style="max-height: 50px; overflow: hidden;"><?php echo $row->post_title; ?></span>
													</a><?php
													$j++;
												}?>
											</div>
							...
							$temp_timestamp += $step;
							$check_day = date('l',$temp_timestamp);
							$month = date('m',$temp_timestamp);
							$i++;
						}?>
					</div>
	...
	public function wtd_get_month(){
        ...
        $next_month_name = date('F', strtotime($_POST['year'].'-'.$next_month.'-1'));
        $prev_month_name = '';
        if ($first_day != date('Y-n-1'))
            $prev_month_name = date('F', strtotime($_POST['year'].'-'.$prev_month.'-1'));
		ob_start();?>
		<div class="wtd_row"><?php
			$current_day_week = date('l', $first_timestamp);
			$current_timestamp = $first_timestamp;
			...
			while ($check_day != 'Sunday' || $month != $next_month){
				...
				switch ($type):
					...
					endswitch;
					$not_month = '';
					if ($month != date('m',$first_timestamp))
							$not_month = 'not_month';
					?>
		...
		<?php 
			...
			$return = array(
				...
				'prev_year' => $prev_year,
                'prev_month_text' => $prev_month_name ? '&laquo; '.$prev_month_name : '&nbsp;',
	...
	public function add_meta_box($post_type){
        ...
        $wtd_pages = get_option('wtd_pages');
        if (empty($wtd_pages['specials_pages']))
            $wtd_pages['specials_pages'] = array();
				...
				<div id="wtd_style">
					<table class="form-table">
						<tr>
							...
							<td>
								<select name="wtd_plugin[specials_type]" id="wtd_pagestyle">
									<option value="1" <?php if (!empty($wtd_plugin['specials_type'])) echo (1 == $wtd_plugin['specials_type']) ? 'selected="selected"' : ''; ?>>One Image</option>
									<option value="2" <?php if (!empty($wtd_plugin['specials_type'])) echo (2 == $wtd_plugin['specials_type']) ? 'selected="selected"' : ''; ?>>Four Images</option>
									<option value="2" <?php if (!empty($wtd_plugin['specials_type'])) echo (3 == $wtd_plugin['specials_type']) ? 'selected="selected"' : ''; ?>>Events List</option>
								</select>
							</td>
						</tr>
					</table>
...
$wtd_specials_page = new wtd_specials_page();

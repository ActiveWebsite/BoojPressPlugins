...
class wtd_activities_page{
	...
	public function __construct(){
		...
		//Ajax calls
		add_action('wp_ajax_get_activities',array(&$this,'get_activities'));
		add_action('wp_ajax_nopriv_get_activities',array(&$this,'get_activities'));
	...
	public function page_content($content){
		...
		if (in_array($post->ID,$wtd_pages['activities_pages'])){
            ...
            global $wpdb;
            global $bookable_categories;
            $bookable_categories = array();
            $wtd_terms = array(1,2,3,4);
            $categories = $wpdb->get_results(
                'SELECT term_id FROM '.$wpdb->prefix.'wtd_meta WHERE wtd_term_id IN ('.implode(',',$wtd_terms).') AND meta_key = "cat_id" LIMIT '.count($wtd_terms)
            );
            foreach ($categories as $c){
                $bookable_categories[] = $c->term_id;
                $children = get_terms('wtd_category',array('parent' => $c->term_id));
                if (!empty($children))
                    foreach($children as $child)
                        $bookable_categories[] = $child->term_id;
            }

            remove_filter('the_content', 'theme_formatter', 99);
            remove_filter ('the_content', 'wpautop');

			if(!empty($wtd_plugin['act_page_mosaic']) && empty($wp_query->query['wtdc'])){

				$content = $this->matrix($content);
				
			}elseif(empty($wtd_plugin['act_page_mosaic']) && empty($wp_query->query['wtdc'])){

				switch ($wtd_plugin['act_page_type']):
					
					case 1:
						$content = $this->big_menu($content);
						break;
					case 2:
						$content = $this->left_side_menu($content);
						break;
					case 3:
						$content = $this->left_side_menu($content);
						break;
			...
			}elseif(!empty($wp_query->query['wtdc'])){


                $exclude = array();
                if (!empty($wtd_plugin['activities_options'][$res_id]))
                    foreach ($wtd_plugin['activities_options'][$res_id] as $cat_id => $cat){
                        if (!empty($cat['parent']))
                            $exclude[] = $cat_id;
                        if(!empty($cat['children']))
                            foreach($cat['children'] as $child_id => $child)
                                $exclude[] = $child_id;
                    }

                $exclude = array_merge($exclude,$wtd_excluded_categories);
                $term = get_term_by('slug',$wp_query->query['wtdc'],'wtd_category');
                if(!is_wp_error($term) && !empty($term)) {
                    if (in_array($term->term_id, $exclude))
                        echo '<style>body{display:none;}</style><script>window.location="' . get_permalink($post->ID) . '"</script>';
                }else{
                    echo '<style>body{display:none;}</style><script>window.location="' . get_permalink($post->ID) . '"</script>';
                }
	...
	private function big_menu($content){
		global $wtd_plugin;
		global $post;
		global $wp_query;
		global $wpdb;
        global $wtd_excluded_categories;
        $res_id = get_post_meta($post->ID,'res_id',true);
		$cur_page_url = get_permalink($post->ID);
		ob_start();
		?>
		<link rel="stylesheet" href="<?php echo WTD_PLUGIN_URL.'assets/css/wtd_activities_page.css?wtd_version='.WTD_VERSION; ?>" media="screen"/>
		<style>
			<?php if (!empty($wtd_plugin['fm_parent_menu'])):?>
				#wtd-navigator .wtd_parent_menu span a{
					font-size: <?php echo $wtd_plugin['fm_parent_menu']['font-style'];?> !important;
					font-weight: <?php echo $wtd_plugin['fm_parent_menu']['font-weight'];?> !important;
					color: <?php echo $wtd_plugin['fm_parent_menu']['color'];?> !important;
				}
			<?php endif; ?>
			<?php if (!empty($wtd_plugin['fm_parent_menu_active'])):?>
				#wtd-navigator .wtd_parent_menu span.active a,#wtd-navigator .wtd_parent_menu span:hover a{
					color:#fff !important;
					font-size: <?php echo $wtd_plugin['fm_parent_menu_active']['font-style'];?> !important;
					font-weight: <?php echo $wtd_plugin['fm_parent_menu_active']['font-weight'];?> !important;
				}
				#wtd-navigator .wtd_parent_menu span.active,#wtd-navigator .wtd_parent_menu span:hover{
					background-color: <?php echo $wtd_plugin['fm_parent_menu_active']['color'];?> !important;
				}
			<?php endif; ?>
			<?php if (!empty($wtd_plugin['fm_child_menu'])):?>
				#wtd-navigator .wtd_menu .wtd_menu_item a{
					font-size: <?php echo $wtd_plugin['fm_child_menu']['font-style'];?> !important;
					font-weight: <?php echo $wtd_plugin['fm_child_menu']['font-weight'];?> !important;
					color: <?php echo $wtd_plugin['fm_child_menu']['color'];?> !important;
				}
			<?php endif; ?>
			<?php if (!empty($wtd_plugin['fm_child_menu_active'])):?>
				#wtd-navigator .wtd_menu .wtd_menu_item a.active,#wtd-navigator .wtd_menu .wtd_menu_item a:hover{
					background-color: <?php echo $wtd_plugin['fm_child_menu_active']['color'];?> !important;
					font-size: <?php echo $wtd_plugin['fm_child_menu_active']['font-style'];?> !important;
					font-weight: <?php echo $wtd_plugin['fm_child_menu_active']['font-weight'];?> !important;
					color:#fff !important;
				}
			<?php endif; ?>
			<?php if (!empty($wtd_plugin['fm_child_menu_background'])):?>
				#wtd-navigator .wtd_menu{
					background-color: <?php echo $wtd_plugin['fm_child_menu_background']['color'];?> !important;
				}
			<?php endif; ?>
		</style>
		<script>
			var cur_category;
			var wtd_base_link = '<?php echo get_post_permalink($post->ID);?>';
			function wtd_get_results(cat_id){
				jQuery.ajax({
					type: 'post',
					data:{
					...
					url: wtd_ajax_url,
					beforeSend:function(){
						jQuery('#wtd_message').show();
						jQuery('#results_container').hide();
					},
					success: function(data){
						jQuery('#wtd_message').hide();
						jQuery( "#results_container" ).html(data);
						jQuery( "#results_container" ).show();
					}
				});
			}
			jQuery(document).ready(function(){
				jQuery('#wtd-navigator span').hover(function(event){
					event.preventDefault();
				});
				jQuery('#wtd-navigator span a').hover(function(event){
					event.preventDefault();
				});
				jQuery(document).on('click','a.page-numbers',function(event){
					event.preventDefault();
					var href = jQuery(this).attr('href');
					var paged = href.split('<?php echo admin_url('admin-ajax.php').'/page/';  ?>');
					var page = parseInt(paged[1]);
					jQuery.ajax({
						type: 'post',
						url: wtd_ajax_url,
						data:{
								action: 'get_activities',
								term_id: cur_category,
								page: page,
								res_id: <?php echo $res_id;?>
							},
						beforeSend:function(){
				...
				<?php 
					if (!empty($wp_query->query['wtdc'])){
						?>
						cur_category = jQuery('a[data-slug="<?php echo $wp_query->query['wtdc'];?>"]').attr('data-cat');
						if (!cur_category)
							cur_category =  jQuery('.wtd_parent_menu_item:first-child()').find('a').attr('data-cat');
						<?php 
					}else{
						?>
						cur_category =  jQuery('.wtd_parent_menu_item:first-child()').find('a').attr('data-cat');
				...
				if (jQuery('[data-cat="'+cur_category+'"]').hasClass('parent_header_link')){
                    jQuery('[data-cat="' + cur_category + '"]').parent().addClass('active');
                    var slug = jQuery('[data-cat="' + cur_category + '"]').attr('data-slug');
                    var show = jQuery('[data-cat="' + cur_category + '"]').parent().attr('data-show');
                    jQuery(show).slideToggle();
                    if (window_width <= 769) {
                        jQuery('#wtd_parent_dropdown option[data-slug="'+slug+'"]').attr('selected', 'selected');
                        jQuery('#drop_parent_' + cur_category).show();
                    }
                    var parent_cat = cur_category;
				}else if(jQuery('[data-cat="'+cur_category+'"]').hasClass('child_cat')){
                    var parent = jQuery('[data-cat="' + cur_category + '"]').parent().parent().parent();
                    var show = parent.attr('id');
                    var parent_cat = jQuery('[data-show="#' + show + '"] a').attr('data-cat');
                    parent.slideToggle();
                    jQuery('[data-cat="' + cur_category + '"]').addClass('active');
                    jQuery('[data-cat="' + parent_cat + '"]').parent().addClass('active');
                    var parent_slug = jQuery('[data-cat="' + parent_cat + '"]').attr('data-slug');
                    var child_slug = jQuery('[data-cat="' + cur_category + '"]').attr('data-slug');
                    if (window_width <= 769) {
                        jQuery('#drop_parent_' + parent_cat).show();
                    }
                    jQuery('#drop_parent_' + parent_cat + ' option[data-slug="' + child_slug + '"]').attr('selected', 'selected');
                    jQuery('#wtd_parent_dropdown option[data-slug="' + parent_slug + '"]').attr('selected', 'selected');
				}
				wtd_get_results(cur_category);
                ...
                jQuery(window).resize(function(){
                    ...
                    if (ww <= 769)
                        ...
                        jQuery('#drop_parent_' + parent_cat).hide();
                ...
                jQuery('.wtd_category_dropdown').change(function(){
                    var slug = jQuery(this).find('option:selected').attr('data-slug');
                    window.location = "<?php echo get_post_permalink($post->ID).'category/';?>" + slug;
                });
			});
		</script>
        <?php
        $mapped_categories = get_option('wtd_mapped_categories');
        $parent_cats = $mapped_categories['activities'];
        $exclude = array();
        ...
        foreach ($not_allowed as $n)
            $exclude[] = $n->term_id;
        foreach ($parent_cats as $pcat_id => $pcat){
            if (!empty($wtd_plugin['activities_options'][$res_id][$pcat_id]['parent']) || in_array($pcat_id,$exclude))
                unset($parent_cats[$pcat_id]);
            else
                foreach ($pcat['children'] as $child_id => $child)
                    if (!empty($wtd_plugin['activities_options'][$res_id][$pcat_id]['children'][$child_id]) || in_array($child_id,$exclude))
                        unset($parent_cats[$pcat_id][$child_id]);
        }
        ?>
		<div id="wtd-navigator">
            <select id="wtd_parent_dropdown" class="wtd_category_dropdown">
                <option>Select Category</option>
                <?php
                    foreach ($parent_cats as $c)
                        echo '<option data-slug="'.$c['parent']->slug.'">'.$c['parent']->name.'</option>';
                ?>
            </select>
			<div class="wtd_parent_menu">
				<div class="wtd_row">
					...
					foreach($parent_cats as $parent_cat){
						...
						?>
						<span id="parent_<?php echo $parent_cat['parent']->term_id;?>_header" class="wtd_parent_menu_item" data-show="#parent_<?php echo $parent_cat['parent']->term_id;?>">
							<a data-show="#parent_<?php echo $parent_cat['parent']->term_id;?>" data-slug="<?php echo $parent_cat['parent']->slug; ?>" data-cat="<?php echo $parent_cat['parent']->term_id;?>" class="parent_header_link" href="<?php echo $cur_page_url.'category/'.$parent_cat['parent']->slug;?>"><?php
								echo $parent_cat['parent']->name;?>
							</a>
						</span><?php
						$count++;
					}?>
			...
			foreach($parent_cats as $parent_cat){
                $categories = $parent_cat['children'];
                ?>
                <select id="drop_parent_<?php echo $parent_cat['parent']->term_id;?>" class="wtd_category_dropdown">
                    <option>Select Subcategory</option>
                    <?php
                        if (count($categories))
                            foreach ($categories as $c)
                                echo '<option data-slug="'.$c->slug.'">'.$c->name.'</option>';
                    ?>
                </select>
				<div id="parent_<?php echo $parent_cat['parent']->term_id;?>" class="wtd_menu">
					<div class="wtd_row">
					<?php
                    if(count($categories) > 0){
							for($i = 0; $i < count($categories); $i++){
								...
								<span class="wtd_menu_item">
									<a id="child_<?php echo $cat->term_id;?>" class="child_cat" data-cat="<?php echo $cat->term_id;?>" data-slug="<?php echo $cat->slug; ?>" href="<?php echo $cur_page_url.'category/'.$cat->slug;?>"><?php
										echo $cat->name;?>
									</a>
								</span><?php
							}
	...
	private function matrix($content){
		global $wtd_plugin;
		global $wpdb;
		global $post;
		global $wp_query;
        ...
        foreach ($not_allowed as $n)
            $exclude[] = $n->term_id;
        $exclude = array_merge($exclude,$wtd_excluded_categories);
		$parent_cats = get_terms('wtd_category',array('parent' => 0,'post_type' => array('wtd_activity'),'exclude' => $exclude,'cache_domain' => 'wtd_activities_'.$res_id,'resorts' => array($res_id)));
		?>

		<div id="wtd_parent_menu">
		<?php
		foreach($parent_cats as $parent_cat){
			$img_sql = $wpdb->prepare(
				'SELECT meta_value FROM '.$wpdb->prefix.'wtd_meta WHERE meta_key = "cat_img" AND term_id = %s LIMIT 1',
				$parent_cat->term_id
			);
			$image = $wpdb->get_var($img_sql);
			if ($image):
			?>
			<div class="masonry-entry">
				<div class="masonry-thumbnail">
					<a href="<?php echo get_post_permalink($post->ID).'category/'.$parent_cat->slug; ?>" title="<?php echo $parent_cat->name;?>">
						<img src="<?php echo WTD_IMG_BASE.$image;?>" class="masonry-thumb"/>
					</a>
				</div>
				<div class="masonry-details">
					<h5>
						<a href="<?php echo get_post_permalink($post->ID).'category/'.$parent_cat->slug; ?>" title="<?php echo $parent_cat->name;?>"><span class="masonry-post-title"><?php echo $parent_cat->name; ?></span></a>
					</h5>
				</div>
			</div><?php 
			endif;
	...
	private function left_side_menu($content){
		global $wtd_plugin;
		global $wpdb;
		global $post;
		global $wp_query;
        ...
        ?>
		<div id="wtd_parent_menu">
			<?php foreach ($parent_cats as $parent_id => $parent):
	...
	public function get_activities(){
        ...
        global $wtd_excluded_categories;
		?>
		<div id="wtd_listing_sc_container">
		<?php
            $exclude = array();
            if (!empty($wtd_plugin['activities_options'][$res_id]))
                foreach ($wtd_plugin['activities_options'][$res_id] as $cat_id => $cat){
            ...
            $exclude = array_merge($exclude,$wtd_excluded_categories);
            $resort = get_term($_POST['res_id'],'wtd_resort');
			...
			$args = array(
				...
				'tax_query' => array(
					...
					array(
						'taxonomy' => 'wtd_resort',
						'field' => 'id',
						'terms' => array($_POST['res_id']),
						'operator' => 'IN'
					)
				)
			);
			
			switch ($wtd_plugin['activities_sort_by']){
				
				case 1 : 
					$args['orderby'] = 'title';
					$args['order'] = 'ASC';
					add_filter('posts_join',array(new wtd_query_filter(),'post_vendor_join'));
					add_filter('posts_orderby',array(new wtd_query_filter(),'post_vendor_order_first'));
					$query = new WP_Query($args);
					remove_filter('posts_join',array(new wtd_query_filter(),'post_vendor_join'));
					remove_filter('posts_orderby',array(new wtd_query_filter(),'post_vendor_order_first'));
					break;
					
				case 2:
                    $args['meta_key'] = 'distance';
				...
				case 3:
					add_filter('posts_join',array(new wtd_query_filter(),'post_vendor_join'));
					add_filter('posts_orderby',array(new wtd_query_filter(),'post_vendor_order_first'));
					add_filter('posts_orderby',array(new wtd_query_filter(),'post_vendor_name_order'));
					$query = new WP_Query($args);
					remove_filter('posts_join',array(new wtd_query_filter(),'post_vendor_join'));
					remove_filter('posts_orderby',array(new wtd_query_filter(),'post_vendor_order_first'));
					remove_filter('posts_orderby',array(new wtd_query_filter(),'post_vendor_name_order'));
					break;
			...
			if($query->found_posts):
                    ...
                    $vendor =  $wpdb->get_row(
                        $wpdb->prepare(
                            'SELECT p.* FROM '.$wpdb->posts.' p
								JOIN '.$wpdb->prefix.'postmeta pmm
								JOIN '.$wpdb->prefix.'wtd_meta wm ON wm.meta_key = "vend_id" AND wm.wtd_term_id = pmm.meta_value AND p.ID = wm.term_id
							WHERE
								pmm.post_id = %s
								AND pmm.meta_key = "vend_id"
                            LIMIT 1',
                            $activity->ID
                        )
                    );
					$metas = $wpdb->get_results($wpdb->prepare('SELECT * FROM '.$wpdb->postmeta.' WHERE post_id = %s',$activity->ID));
                    foreach ($metas as $key => $meta){
						$name = $meta->meta_key;
						$$name = maybe_unserialize($meta->meta_value);
					}
					?>
					<div class="wtd_listing_container <?php if($i == 0) echo 'wtd_top_15'; ?>">
						<div class="wtd_listing_sc_listing_list">
							<div class="wtd_listing_sc_top_content clearfix">
							<?php 
								$listing_url = get_post_permalink($activity->ID);

                                $title = '<span class="wtd_listing_title">'.$activity->post_title.'</span>';
                                if (!empty($title) && !empty($vendor->post_title) && !substr_count($title,$vendor->post_title))
                                    $title .= '<span class="wtd_listing_comma"></span><span class="wtd_listing_vendor">'.$vendor->post_title.'</span>';
								elseif (!empty($title) && !empty($vendor->post_title) && substr_count($title,$vendor->post_title)> 0)
									$title .= '<span class="wtd_listing_vendor_hidden">'.$vendor->post_title.'</span>';
                                elseif (empty($title) && !substr_count($title,$vendor->post_title))
                                    $title .= '<span class="wtd_listing_vendor">'.$vendor->post_title.'</span>';
							?>
							<p class="wtd_listing_title_bar"><a href="<?php echo $listing_url;?>"><?php echo $title;?></a></p>
								<div class="wtd_listing_sc_imageArea">
                                    <a href="<?php echo $listing_url;?>">
									    <img src="<?php echo wtd_get_image_url($listing_thm);?>" alt="<?php echo $activity->post_title;?>"/>
                                    </a>
								</div>
								<div class="wtd_listing_sc_centerContent">
                                    <?php if (!empty($wtd_plugin['wtd_booking_button']) && in_array($_POST['term_id'],$bookable_categories)): ?>
                                     <a href="<?php echo WTD_LEAD_BASE.$list_id;?>" target="_blank" class="wtd_booking_button">Book Now</a>
                                    <?php endif;?>
									<ul>
										<li>
											<p><?php
												$desc = $activity->post_content;
                                                wtd_excerpt_generator($desc, $activity);
                                                ?>
											</p>
										</li>
									</ul>
				...
				if($pagination):?>
					<div class="wtd_listing_sc_pagination"><?php
						echo $pagination;?>
					</div><?php
				endif;
			else:?>
	...
	private function results($content){
		...
		$res_id = get_post_meta($post->ID,'res_id',true);
		$category = get_term_by('slug',$wp_query->query['wtdc'],'wtd_category');
        $mapped_categories = get_option('wtd_mapped_categories');
        $parent_cats = $mapped_categories['activities'];
        $exclude = array();
        $not_allowed = $wpdb->get_results(
        ...
        <div id="wtd_breadcrumb">
            ...
            <?php
                $first_cat = $category;
                if ($category->parent)
                    $first_cat = get_term($category->parent,'wtd_category');
                if ($wtd_plugin['act_page_type'] == 2):?>
                <select class="wtd_subcategory_navigator">
                    <option>Select Category</option>
                    <?php
                        foreach ($parent_cats as $parent_id => $parent){
                            $scat = $parent['parent'];
                            $selected = '';
                            if ($scat->term_id == $first_cat->term_id)
                                $selected = ' selected="selected"';
                            echo '<option data-slug="'.$scat->slug.'"'.$selected.'>'.$scat->name.'</option>';
                        }
                    ?>
                </select>
                <?php else: ?>
                <a id="parent_<?php echo $first_cat->term_id;?>_header" class="wtd_pull_left" href="<?php echo get_post_permalink($post->ID).'category/'.$first_cat->slug; ?>" ><?php
                    echo $first_cat->name;?>
                </a>
                <?php endif;?>
                <?php if ($wtd_plugin['act_page_type'] == 2):
                ...
                <span class="wtd_bread_separator">&gt;</span>
                    <select class="wtd_subcategory_navigator">
                        <option>Select Subcategory</option>
                        <?php
                        $subcats = $parent_cats[$parent_category]['children'];
                        foreach ($subcats as $scat){
                            $selected = '';
                            if ($scat->term_id == $category->term_id)
                                $selected = ' selected="selected"';
                            echo '<option data-slug="'.$scat->slug.'"'.$selected.'>'.$scat->name.'</option>';
                        }
                        ?>
                    </select>
                <?php elseif ($category->parent): ?>
                    <span class="wtd_bread_separator">&gt;</span>
                    <a id="parent_<?php echo $category->term_id;?>_header" class="wtd_pull_left" href="<?php echo get_post_permalink($post->ID).'category/'.$category->slug; ?>"><?php
                        echo $category->name;?>
                    </a>
                <?php endif;?>
        </div>
		...
		if ($category && !is_wp_error($category)):
			...
			if ($wtd_plugin['act_page_type'] == 3):
				$wtd_parent_id = $category->parent ? $category->parent : $category->term_id;
                $subcategories = $parent_cats[$wtd_parent_id]['children'];
				$masonry_item = 'wtd_page_results';
				?>
				<ul class="wtd_subcategory_menu">
					<?php foreach ($subcategories as $scat): ?>				
					<li class="wtd_subcategory_menu_item <?php echo ($scat->slug == $wp_query->query['wtdc']) ? 'active' : '';?>">
							<a href="<?php echo get_post_permalink($post->ID).'category/'.$scat->slug; ?>"><?php echo $scat->name;?></a>
					</li>
					<?php endforeach; ?>
				</ul>
			<?php endif;?>
		...
		<?php
            ...
            $exclude = array_merge($exclude,$wtd_excluded_categories);
			$paged = !empty($_GET['pagenr']) ? $_GET['pagenr'] : 1;
			$args = array(
				'posts_per_page' => 10,
				'paged' => $paged,
				'post_type' => 'wtd_activity',
				'tax_query' => array(
					array(
						'taxonomy' => 'wtd_category',
						'field' => 'slug',
						'terms' => array($wp_query->query['wtdc']),
						'operator' => 'IN'
					),
					array(
						'taxonomy' => 'wtd_category',
			...
			if($query->found_posts):
				...
				foreach ($query->posts as $activity):
					...
					?>
					<div class="wtd_listing_container <?php echo $masonry_item;?>">
						<div class="wtd_listing_sc_listing_list">
							<div class="wtd_listing_sc_top_content clearfix">
							<?php 
                            $listing_url = get_post_permalink($activity->ID);
							...
							<p class="wtd_listing_title_bar"><a href="<?php echo $listing_url;?>"><?php echo $title;?></a></p>
								...
								<div class="wtd_listing_sc_centerContent">
                                    <?php if (!empty($wtd_plugin['wtd_booking_button']) && in_array($category->term_id,$bookable_categories)): ?>
                                        <a href="<?php echo WTD_LEAD_BASE.$list_id;?>" target="_blank" class="wtd_booking_button">Book Now</a>
                                    <?php endif;?>
									<ul>
										<li>
	...
	public function add_meta_box($post_type){
        ...
        $wtd_pages = get_option('wtd_pages');
        if (empty($wtd_pages['activities_pages']))
            $wtd_pages['activities_pages'] = array();
        if(!empty($post) && !is_wp_error($post))
        if (in_array($post->ID,$wtd_pages['activities_pages'])){
            add_meta_box(
                'activities_box',
                'Super Advanced What To Do Page Builder',
                array( &$this, 'meta_box' ),
                $post_type,
                'advanced',
                'high'
	...
	public function meta_box($post){
		...
		global $wtd_plugin;
        global $wtd_excluded_categories;
		$options = get_post_meta($post->ID,'wtd_page_options',true);
		$place = 'activities_options';
		$resort = get_post_meta($post->ID,'res_id',true);
		?>
		<link rel="stylesheet" href="<?php echo WTD_PLUGIN_URL . 'includes/framework/ReduxCore/assets/js/vendor/select2/select2.css';?>"/>
		<link rel="stylesheet" href="<?php echo WTD_PLUGIN_URL . 'assets/css/jquery-ui-theme.css';?>"/>
		<link rel="stylesheet" href="<?php echo WTD_PLUGIN_URL . 'assets/css/wtd_admin.css';?>"/>
		<link rel="stylesheet" href="<?php echo WTD_PLUGIN_URL . 'includes/framework/ReduxCore/assets/css/vendor/elusive-icons/elusive-webfont.css';?>"/>
		<script src="https://code.jquery.com/ui/1.11.2/jquery-ui.min.js"></script>
		<script src="<?php echo WTD_PLUGIN_URL . 'includes/framework/ReduxCore/assets/js/vendor/select2/select2.min.js';?>"></script>
		<script src="<?php echo WTD_PLUGIN_URL . 'assets/js/wtd_admin.js';?>"/></script>
				
		<div id="wtd_metabox" class="wtd_admin">
			<div id="wtd_metabox_content" style="display:block;">
				<p>Changes here only apply to this page, not the entire site like they do if you make changes in the What To Do Admin screen.</p>
				<ul>
					<li>
						<a href="#wtd_categories">Categories</a>
					</li>
					<li>
						<a href="#wtd_style">Style</a>
					</li>
				</ul>
				<div id="wtd_categories">
					<table class="form-table">
						<tr>
							<th>
								<label for="wtd_categories">
									Categories
								</label>
								<div id="<?php echo $place.'_inputs';?>">
									<?php 
										if (!empty($wtd_plugin[$place][$resort]))
											foreach($wtd_plugin[$place][$resort] as $cat_id => $value){
												if (!empty($value['parent']))
													echo '<input type="hidden" id="wtd_page_category_'.$cat_id.'" name="wtd_plugin['.$place.']['.$resort.']['.$cat_id.'][parent]" value="1" />';
												if (is_array($value['children']))
													foreach ($value['children'] as $child_id => $val)
														echo '<input type="hidden" id="wtd_page_category_'.$child_id.'" name="wtd_plugin['.$place.']['.$resort.']['.$cat_id.'][children]['.$child_id.']" value="1" />';
														
											}
									?>
								</div>
							</th>
							<td class="wtd_button_grid">
								<?php
                                    $not_allowed = $wpdb->get_results(
                                        'SELECT term_id FROM '.$wpdb->prefix.'wtd_meta WHERE meta_key = "cat_id" AND wtd_term_id IN (6,903,1008) LIMIT 3'
                                    );
									...
									foreach ($cats as $key => $val){
									    $category = $val['parent'];
										echo '<a href="javascript:void(0);" class="button wtd_cat" data-term="'.$category->term_id.'"><span>'.$category->name.'</span></a>';
									}
								?>
							</td>
						</tr>
						<tr class="child_container">
							<th>
								<label for="wtd_terms">
									Subcategories
								</label>
							...
							<td>
								<?php 
									foreach ($cats as $key => $val){
										...
										echo '<div id="wtd_children_'.$category->term_id.'" class="wtd_children wtd_button_grid">';
										$icon = 'el-icon-check';
										$class = 'button-primary';
										$btn_text = 'Enabled';
										$parent_disabled = false;
										if (!empty($wtd_plugin[$place][$resort][$category->term_id]['parent'])){
											$parent_disabled = true;
											$icon = "el-icon-check-empty";
											$class = '';
											$btn_text = 'Disabled';
										}
										echo '<a href="javascript:void(0);" class="button wtd_cat_disabler '.$class.' wtd_toggle" data-term="'.$category->term_id.'" data-place="'.$place.'" data-resort="'.$resort.'"><span>'.$btn_text.'</span><i class="'.$icon.'"></i></a><hr/>';
										foreach ($children as $child){
											$icon = 'el-icon-check';
											$class = 'button-primary';
											if ($parent_disabled || !empty($wtd_plugin[$place][$resort][$category->term_id]['children'][$child->term_id])){
												$icon = "el-icon-check-empty";
												$class = '';
											}
											echo '<a href="javascript:void(0);" class="button wtd_scat '.$class.' wtd_toggle" data-parent="'.$category->term_id.'" data-term="'.$child->term_id.'" data-place="'.$place.'" data-resort="'.$resort.'"><span>'.$child->name.'</span><i class="'.$icon.'"></i></a>';
										}
										echo '</div>';
									}
								?>
				...
				<div id="wtd_style">
					<table class="form-table">
						<tr>
							<th>
								<label for="wtd_display">
									Navigation
								</label>
							</th>
							<td>
								<select name="wtd_plugin[act_page_type]" id="wtd_pagestyle">
									<option value="1" <?php if (!empty($wtd_plugin['act_page_type'])) echo (1 == $wtd_plugin['act_page_type']) ? 'selected="selected"' : ''; ?>>Full Menu</option>
									<option value="2" <?php if (!empty($wtd_plugin['act_page_type'])) echo (2 == $wtd_plugin['act_page_type']) ? 'selected="selected"' : ''; ?>>Top Drop-Down Menu</option>
									<option value="3" <?php if (!empty($wtd_plugin['act_page_type'])) echo (3 == $wtd_plugin['act_page_type']) ? 'selected="selected"' : ''; ?>>Left Side Menu</option>
								</select>
							</td>
						</tr>
						<tr>
							...
							<td>
								<select name="wtd_plugin[act_page_mosaic]" id="wtd_display">
									<option value="0" <?php if (isset($wtd_plugin['act_page_mosaic'])) echo (0 == $wtd_plugin['act_page_mosaic']) ? 'selected="selected"' : ''; ?>>Disable</option>
									<option value="1" <?php if (isset($wtd_plugin['act_page_mosaic'])) echo (1 == $wtd_plugin['act_page_mosaic']) ? 'selected="selected"' : ''; ?>>Enable</option>
								</select>
							</td>
						</tr>
					</table>
		...
		<script>
			jQuery('#wtd_metabox_content').tabs();
		</script>
		<?php
	...
	public function save_page($post_id){
		global $wtd_plugin;
		if (!empty($_POST['wtd_plugin'])){
			foreach ($_POST['wtd_plugin'] as $key => $value)
				$wtd_plugin[$key] = $value;
			update_option('wtd_plugin',$wtd_plugin);
		}
        if(!empty($_POST['wtd_coupon_code']))
            update_post_meta($post_id,'coupon_code',$_POST['wtd_coupon_code']);
	}

}

$wtd_activities_page = new wtd_activities_page();

...
class wtd_map_page{
    ...
    public function __construct(){
        ...
        //Page Content
        if (!empty($wtd_plugin['map-page']))
            add_action('the_content', array(&$this, 'page_content'), 99);

        //Ajax calls
        add_action('wp_ajax_get_coordinates',array(&$this,'get_coordinates'));
        add_action('wp_ajax_nopriv_get_coordinates',array(&$this,'get_coordinates'));
    ...
    public function big_menu($content){
        global $wtd_plugin;
        global $post;
        global $wp_query;
        global $wpdb;
        ...
        $exclude = array();
        if (!empty($wtd_plugin['map_options'][$res_id]))
            foreach ($wtd_plugin['map_options'][$res_id] as $cat_id => $cat){
                if (!empty($cat['parent']))
                    $exclude[] = $cat_id;
            }
        ...
        $cats = get_terms('wtd_category',array('post_type' => array('wtd_activity'),'exclude' => $exclude,'cache_domain' => 'wtd_activities_'.$res_id,'resorts' => array($res_id)));
        $parent_cats = array();
        foreach ($cats as $c){
            if (!$c->parent){
                if (empty($parent_cats[$c->term_id])) {
                    $parent_cats[$c->term_id] = array(
                        'parent' => $c,
                        'children' => array()
                    );
                }elseif(empty($parent_cats[$c->term_id]['parent'])){
                    $parent_cats[$c->term_id]['parent'] = $c;
                }
            }else{

                if (empty($parent_cats[$c->parent])) {
                    $parent_cats[$c->parent] = array(
                        'parent' => array(),
                        'children' => array( $c )
                    );
                }elseif(empty($parent_cats[$c->parent]['children'])){
                    $parent_cats[$c->term_id]['children'] = array( $c );
                }elseif(!empty($parent_cats[$c->term_id]))
                    if(!in_array($c,$parent_cats[$c->term_id]['children']) && !empty($parent_cats[$c->term_id]['children'])){
                        $parent_cats[$c->term_id]['children'][] = $c;
                    }
        ...
        foreach ($parent_cats as $parent_id => $parent_cat){
            if (empty($parent_cat['parent'])) {
                $parent = get_term($parent_id, 'wtd_category');
                if ($parent->parent) {
                    foreach ($parent_cat['children'] as $child)
                        $parent_cats[$parent->parent]['children'][] = $child;
                    unset($parent_cats[$parent_id]);
                }
                else
                    $parent_cats[$parent_id]['parent'] = $parent;
            }
        }
        $lat = !empty($wtd_plugin['user_map']['lat']) ? $wtd_plugin['user_map']['lat'] : '28';
        $lng = !empty($wtd_plugin['user_map']['lng']) ? $wtd_plugin['user_map']['lng'] : '-95.677068';
        ?>
        <style>
            #map_canvas img{
                max-width: none;
        ...
        <link rel="stylesheet" href="<?php echo WTD_PLUGIN_URL.'assets/css/wtd_activities_page.css?wtd_version='.WTD_VERSION;?>" type="text/css" media="screen" />
        <style>
        ...
        <script>
            ...
            var wtd_maps_ajax = "<?php echo admin_url('/admin-ajax.php');?>";
            ...
            jQuery(document).ready(function(){
                ...
                jQuery(document).on('click','#wtd-navigator .wtd_parent_menu span',function(event){
                    event.preventDefault();
                    var slug = jQuery(this).find('a').attr('data-slug');
                    window.location = wtd_base_link + '/category/' + slug + '/';
                });
                jQuery(document).on('click','#wtd-navigator .wtd_contents div a',function(event){
                    event.preventDefault();
        ...
        ?>
                ...
                wtd_get_coordinates(cur_category);
        ...
        <script>
            function wtd_get_coordinates(cat_id){
                jQuery.ajax({
                    url: wtd_maps_ajax,
                    type: 'post',
                    dataType: 'json',
            ...
            function initialize() {
                ...
                var map_options = {
                    ...
                    mapTypeControl: true,
                    scaleControl: true,
                    center: new google.maps.LatLng(lat, lng,4),
                    zoom: map_zoom,
            ...
            function add_marker(value){
                ...
                marker.infowindow = new google.maps.InfoWindow({
                    content: '<img src="'+value.image+'" class="marker_image"/><span class="location_title">'+value.title+'</span><div>'+value.desc+'</div>',
                    position: location,
                    disableAutoPan: true
                });
            ...
            <div class="wtd_parent_menu">
                <div class="wtd_row">
                    ...
                    foreach($parent_cats as $parent_cat){
                        ...
                        <a data-show="#parent_<?php echo $parent_cat['parent']->term_id;?>" data-slug="<?php echo $parent_cat['parent']->slug;?>" data-cat="<?php echo $parent_cat['parent']->term_id;?>"  class="parent_header_link" href="<?php echo get_term_link($parent_cat['parent'],'wtd_category');?>">
                            <?php
                        ...
                        <div class="wtd_row">
                            ...
                            if(count($categories) > 0){
                                ...
                                for($i = 0; $i < count($categories); $i++){
                                    ...
                                    <a id="child_<?php echo $cat->term_id;?>" class="child_cat" data-slug="<?php echo $cat->slug;?>" data-cat="<?php echo $cat->term_id;?>" href="<?php echo get_term_link($cat,'wtd_category');?>">
                                        <?php
                                        echo $cat->name;
                                        ?>
                                    </a>
    ...
    public function results($content){
        ...
        <?php
        if ($wtd_plugin['map_page_type'] == 2){
            echo '<p>Please select a category from the dropdowns below to view locations on the map.</p>';
        }
        ?>
        <div id="wtd_breadcrumb">
            <a id="parent_<?php echo $post->ID;?>_header" href="<?php echo get_post_permalink($post->ID);?>" style="float:left;"><?php
                echo $post->post_title;?>
            </a>
            <?php if (!empty($wp_query->query['wtdc'])): ?>
                ...
                if ($wtd_plugin['map_page_type'] == 2):?>
                    <select class="wtd_subcategory_navigator" id="wtd_bread_cats">
                        <option>Select Category</option>
                        <?php
                        foreach ($parent_cats as $scat){
                            $selected = '';
                            if (($scat->term_id == $first_cat->term_id) && !empty($wp_query->query['wtdc']))
                                $selected = ' selected="selected"';
                            echo '<option data-slug="'.$scat->slug.'"'.$selected.' data-cat="'.$scat->term_id.'">'.$scat->name.'</option>';
                        }
                        ?>
                    </select>
                <?php else: ?>
                    <a id="parent_<?php echo $first_cat->term_id;?>_header" href="<?php echo get_post_permalink($post->ID).'category/'.$first_cat->slug; ?>" data-slug="<?php echo $first_cat->slug;?>" data-cat="<?php echo $first_cat->term_id;?>" style="float:left;"><?php
                        echo $first_cat->name;?>
                    </a>
                <?php endif;?>
                <?php if ($wtd_plugin['map_page_type'] == 2):
                    $parent_category = $category->term_id;
                    if ($category->parent)
                        $parent_category = $category->parent;
                    ?>
                    <span style="float:left;margin-left:5px;margin-right:5px;">&gt;</span>
                    <select class="wtd_subcategory_navigator" id="wtd_bread_scats">
                        <option>Select Subcategory</option>
                        <?php
                        $exclude = array();
                        if (!empty($wtd_plugin['map_options'][$res_id]))
                        ...
                        $exclude = array_merge($exclude,$wtd_excluded_categories);
                        $child_cats = get_terms('wtd_category',array('parent' => $parent_category,'post_type' => array('wtd_activity'),'exclude' => $exclude,'cache_domain' => 'wtd_activities_'.$res_id,'resorts' => array($res_id)));
                        foreach ($child_cats as $scat){
                            $selected = '';
                            if ($scat->term_id == $category->term_id && !empty($wp_query->query['wtdc']))
                                $selected = ' selected="selected"';
                            echo '<option data-slug="'.$scat->slug.'"'.$selected.' data-cat="'.$scat->term_id.'">'.$scat->name.'</option>';
                        }
                        ?>
                ...
                <?php elseif ($category->parent): ?>
                    <span style="float:left;margin-left:5px;margin-right:5px;">&gt;</span>
                    <a id="parent_<?php echo $category->term_id;?>_header" href="<?php echo get_post_permalink($post->ID).'category/'.$category->slug; ?>" data-slug="<?php echo $category->slug;?>" data-cat="<?php echo $category->term_id;?>" style="float:left;"><?php
                        echo $category->name;?>
                    </a>
                <?php endif;?>
        ...
        <?php
        if ($wtd_plugin['map_page_type'] == 3){
            echo '<p>Please select a category below to view locations on the map.</p>';
        }
        $masonry_item = '';
        if ($wtd_plugin['map_page_type'] == 3):
            if (!empty($wp_query->query['wtdc'])) {
                ...
                $exclude = array();
                if (!empty($wtd_plugin['map_options'][$res_id][$wtd_parent_id]['children']))
                    foreach ($wtd_plugin['map_options'][$res_id][$wtd_parent_id]['children'] as $cat_id => $cat) {
                        $exclude[] = $cat_id;
                    }
                $subcategories = get_terms('wtd_category', array('parent' => $wtd_parent_id, 'post_type' => array('wtd_activity'), 'exclude' => $exclude, 'cache_domain' => 'wtd_activities_' . $res_id, 'resorts' => array($res_id)));
                ...
                <ul class="wtd_subcategory_menu">
                    <?php foreach ($subcategories as $scat): ?>
                        <li class="wtd_subcategory_menu_item <?php echo ($scat->slug == $wp_query->query['wtdc']) ? 'active' : ''; ?>">
                            <a href="<?php echo get_post_permalink($post->ID) . 'category/' . $scat->slug; ?>" data-slug="<?php echo $scat->slug;?>" data-cat="<?php echo $scat->term_id;?>"><?php echo $scat->name; ?></a>
                        </li>
                    <?php endforeach; ?>
                </ul>
            <?php
                ...
                ?>
                <ul class="wtd_subcategory_menu">
                    <?php foreach ($parent_cats as $scat):?>
                        <li class="wtd_subcategory_menu_item">
                            <a href="<?php echo get_post_permalink($post->ID) . 'category/' . $scat->slug; ?>" data-slug="<?php echo $scat->slug;?>" data-cat="<?php echo $scat->term_id;?>"><?php echo $scat->name; ?></a>
                        </li>
                    <?php endforeach; ?>
                </ul>
        ...
        <div id="wtd_listing_sc_container" class="<?php echo ($wtd_plugin['map_page_type'] == 3) ? 'left_menu_map' : '';?>" >

            <script>
                <?php if (!$category->parent && !empty($wp_query->query['wtdc'])): ?>
                    jQuery('body').hide();
                    window.location = jQuery('.wtd_subcategory_menu a').first().attr('href');
                <?php
                    echo '</script>';
                    die();
                ...
                jQuery(document).ready(function(){
                    ...
                    <?php
                        switch($wtd_plugin['map_page_type']){
                            case 2: echo 'var wtd_node_name = "option";';break;
                            case 3: echo 'var wtd_node_name = "a";';break;
                        }
                        if (!empty($wp_query->query['wtdc'])){

                            ?>
                    cur_category = jQuery(wtd_node_name+'[data-slug="<?php echo $wp_query->query['wtdc'];?>"]').attr('data-cat');
                    if (!cur_category)
                        cur_category =  jQuery(wtd_node_name+'[data-slug]').first().attr('data-cat');
                    wtd_get_coordinates(cur_category);
                    <?php
                }else{
                ?>
            ...
            <div id="map_canvas" class="<?php echo $masonry_item; ?>"></div>

        </div>
        <?php if ($wtd_plugin['wtd_copyright_holder']): ?>
            <div style="clear:both;"></div>
            <link rel="stylesheet" href="<?php echo WTD_PLUGIN_URL; ?>assets/css/wtd_main.css" media="screen"/>
            <div id="wtd_footer">
                <div style="width:100%;float:left;margin-bottom:5px;">
                    <a href="http://www.whattodo.info" target="_blank">
                        <img src="<?php echo WTD_PLUGIN_URL; ?>assets/img/whattodo-logo.png" style="max-width: 140px;float:left;"/>
                    </a>
                </div>
        ...
        <?php else: ?>
            <div style="float:right;margin-right:5px;">&copy; <?php echo date('Y', time());?> <a href="http://www.whattodo.info" target="_blank">WhatToDo.info</a></div>
        <?php endif;
        $content = ob_get_clean();
        return $content;
    ...
    public function get_coordinates(){
        ...
        if (isset($_POST['cat_id'])){
            $icon = $wpdb->get_var($wpdb->prepare('SELECT meta_value FROM '.$wpdb->prefix.'wtd_meta WHERE term_id = %s AND meta_key = "map_icon" LIMIT 1',$_POST['cat_id']));
            $args = array(
                'posts_per_page' => -1,
            ...
            foreach ($results->posts as $result){
                global $wpdb;
                $vendor_metas = $wpdb->get_results(
                    $wpdb->prepare(
                        'SELECT pm.* FROM '.$wpdb->prefix.'postmeta pm
						JOIN '.$wpdb->prefix.'postmeta pmm ON pmm.post_id = %s AND pmm.meta_key = "vend_id"
						JOIN '.$wpdb->prefix.'wtd_meta wm ON wm.meta_key = "vend_id" AND wm.wtd_term_id = pmm.meta_value
						JOIN '.$wpdb->prefix.'posts p ON p.ID = wm.term_id
					WHERE
						pm.post_id = wm.term_id
						AND
						pm.meta_key IN ("lat","lng")
						AND
						pm.meta_value != "0"
						',
                        $result->ID
                ...
                if($vendor_metas){
                    ...
                    $ret[] = array(
                        'lat' => utf8_encode($lat),
                        'lng' => utf8_encode($lng),
                        'title' => utf8_encode($result->post_title),
                        'desc' => strip_tags(utf8_encode(substr(strip_tags($result->post_content)	,0,70).'...')),
                        'image' => utf8_encode(WTD_IMG_BASE.get_post_meta($result->ID,'listing_thm',true)),
                        'url' => utf8_encode(get_post_permalink($result->ID)),
                        'icon' => utf8_encode(WTD_PLUGIN_URL.'assets/img/map_icons/'.$icon)
                    );
                }
            }
    ...
    public function meta_box($post){
        ...
        <?php
            $categories = get_terms('wtd_category',array('parent' => 0,'post_type' => array('wtd_activity'), 'cache_domain' => 'wtd_activities_'.$resort, 'resorts' => array($resort)));
        ...
        <?php
            foreach ($categories as $key => $category){
                $children = get_terms('wtd_category',array('parent' => $category->term_id,'post_type' => array('wtd_activity'), 'cache_domain' => 'wtd_activities_'.$resort, 'resorts' => array($resort)));
        ...
        <td>
        <select name="wtd_plugin[map_page_type]" id="wtd_pagestyle">
        <option value="1" <?php if (!empty($wtd_plugin['map_page_type'])) echo (1 == $wtd_plugin['map_page_type']) ? 'selected="selected"' : ''; ?>>Full Menu</option>
        <option value="2" <?php if (!empty($wtd_plugin['map_page_type'])) echo (2 == $wtd_plugin['map_page_type']) ? 'selected="selected"' : ''; ?>>Top Drop-Down Menu</option>
        <option value="3" <?php if (!empty($wtd_plugin['map_page_type'])) echo (3 == $wtd_plugin['map_page_type']) ? 'selected="selected"' : ''; ?>>Left Side Menu</option>
        </select>
        </td>
        </tr>
        </table>

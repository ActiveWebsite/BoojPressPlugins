...
class wtd_calendar_page{
	...
	public function __construct(){
		...
		//Ajax calls
		add_action('wp_ajax_get_wtd_month',array(&$this,'wtd_get_month'));
		add_action('wp_ajax_nopriv_get_wtd_month',array(&$this,'wtd_get_month'));
		...
		?>
			...
			<script>
				...
				jQuery(document).ready(function(){
					...
					jQuery('#modal_background').on('click', function(event){
						jQuery(this).hide();
            ...
            ?>
			<div class="row" id="calendar_controler">
				<a href="javascript:void(0);" id="prev_month" class="get_month" data-year="<?php echo $prev_year;?>" data-month="<?php echo $prev_month;?>">&nbsp;</a>
				<span id="curent_month"><?php echo date('F Y');?></span>
				<a href="javascript:void(0);" id="next_month" class="get_month" data-year="<?php echo $next_year;?>" data-month="<?php echo $next_month;?>"><?php echo $next_month_name;?> &raquo;</a>
			</div>
			<div id="wtd_events">
				...
				<div id="dates">
					<div class="wtd_row"><?php
						...
						while ($check_day != 'Sunday' || $month != $next_month){
							...
							if (!empty($wtd_plugin['calendar_options'][$res_id])){
								$category = get_term_by('name','Event Calendar','wtd_category');
								$not_categories = array_keys($wtd_plugin['calendar_options'][$res_id]['calendar']['children']);
								$args['tax_query'][] = array(
							...
							switch ($type):
								case 1:
									...
									if (!empty($results)){
										...
										$image = $wpdb->get_var(
											$wpdb->prepare(
												'SELECT meta_value FROM '.$wpdb->prefix.'postmeta WHERE post_id IN '.$in.' AND meta_key = "eve_thm" AND meta_value NOT LIKE %s ORDER BY RAND() LIMIT 1',
												'%default.jpg'
											)
										);
										if (!$image){
									...
									$events_text = count($results).' &nbsp;&nbsp;Events';
									break;
								case 2: 
									$images = '';
									if (!empty($results)){
	...
	public function wtd_get_month(){
		...
		<?php 
			...
			$return = array(
				...
				'prev_year' => $prev_year,
				'prev_month_text' => $prev_month_name,
	...
	public function add_meta_box($post_type){
        ...
        $wtd_pages = get_option('wtd_pages');
        if (empty($wtd_pages['calendar_pages']))
            $wtd_pages['calendar_pages'] = array();
				...
				<div id="wtd_style">
					<table class="form-table">
						<tr>
							...
							<td>
								<select name="wtd_plugin[calendar_type]" id="wtd_pagestyle">
									<option value="1" <?php if (!empty($wtd_plugin['calendar_type'])) echo (1 == $wtd_plugin['calendar_type']) ? 'selected="selected"' : ''; ?>>One Image</option>
									<option value="2" <?php if (!empty($wtd_plugin['calendar_type'])) echo (2 == $wtd_plugin['calendar_type']) ? 'selected="selected"' : ''; ?>>Four Images</option>
									<option value="2" <?php if (!empty($wtd_plugin['calendar_type'])) echo (3 == $wtd_plugin['calendar_type']) ? 'selected="selected"' : ''; ?>>Events List</option>
								</select>
							</td>
						</tr>
					</table>
...
$wtd_calendar_page = new wtd_calendar_page();

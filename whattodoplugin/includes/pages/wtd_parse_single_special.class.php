...
if (!class_exists('wtd_parse_single_special')) {
    class wtd_parse_single_special
    {
        ...
        public function __construct() {
            ...
            //Ajax calls
            add_action('wp_ajax_wtd_build_special', array(&$this, 'build_content'));
            add_action('wp_ajax_nopriv_wtd_build_special', array(&$this, 'build_content'));
        ...
        public function single_special($content)
        {
            ...
            if ($post->post_type == 'wtd_special') {
                if (empty($wp_query->query['wtd_parse_id'])) {
                ...
                <link rel="stylesheet" href="<?php echo WTD_PLUGIN_URL . 'assets/css/wtd_activities_page.css?wtd_version=' . WTD_VERSION; ?>"/>
                <?php

                $base_request = $wtd_connector->get_base_request();
                $base_request['special_id'] = $wp_query->query['wtd_parse_id'];
                ?>
                ...
                if ($iPod || $iPhone || $iPad || $mac):?>
                    <form method="post">
                        ...
                        <button type="submit" class="wtd_ical_button">Add to iCal</button>
                    </form>
                <?php endif ;?>
                ...
                <script>
                    ...
                    var wtd_base_request = <?php echo json_encode($base_request);?>;
                        function wtd_parse_special(){
                            Parse.Cloud.run('get_special', wtd_base_request, {
                ...
                <div class="wtd_event_sc_event_list">
                    <div class="wtd_event_sc_top_content clearfix">
                        ...
                        <div class="wtd_single_event_sc_left_content">
                            ...
                            <p>
                                <?php
                                $eve_timestamp = strtotime($event->specialDate->iso);
                            ...
                            <?php if (!empty($event->phone)):
                                ...
                                if (strlen($event_phone) == 10)
                                    $event_phone = " (" . substr($event_phone, 0, 3) . ") " . substr($event_phone, 3, 3) . "-" . substr($event_phone, 6);
                                ?>
                                <p>
                                    <?php echo $event_phone; ?>
                                </p>
                            <?php endif; ?>
                            <?php if (!empty($event->email) && filter_var($event->email, FILTER_VALIDATE_EMAIL)): ?>
                                <p>
                            ...
                            <?php if (!empty($event->website)): ?>
                                <p>
                                    <a href="<?php echo (substr_count($event->website, 'http://')) ? $event->website : 'http://' . $event->website; ?>"
                                       target="_blank"><?php echo $event->website; ?></a>
                                </p>
                            <?php endif; ?>
                            <?php
                            if (!empty($addresses)):
    ...
    $wtd_parse_single_special = new wtd_parse_single_special();

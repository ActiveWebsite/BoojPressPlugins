...
if(!class_exists('wtd_search_page')){

    class wtd_search_page{

        public function __construct(){

            add_shortcode( 'wtd_search_form', array(&$this,'search_form'));

            //Ajax Search
            add_action('wp_ajax_wtd_quick_search',array(&$this,'quick_search'));
            add_action('wp_ajax_nopriv_wtd_quick_search',array(&$this,'quick_search'));

            $wtd_plugin = get_option('wtd_plugin');
            //Page Content
            if (!empty($wtd_plugin['search-page']))
        ...
        public function search_form( $atts ) {
            ...
            if (!empty($wtd_pages['search_page'])):
                ...
                $res_ids = implode(',',$wtd_plugin['wtd_resorts']);
                $res_no = count($wtd_plugin['wtd_resorts']);
                $resorts_available = $wpdb->get_results('SELECT * FROM '.$wpdb->terms.' WHERE term_id IN ('.$res_ids.') ORDER BY name ASC LIMIT '.$res_no);
                ...
                <form method="get" class="wtd_search_form" action="<?php echo get_permalink($wtd_pages['search_page'][0]);?>">
                    <?php
                    if (!empty($a['exclude'])) {
                        $exclude = explode(',',$a['exclude']);
                        foreach ($exclude as $ex)
                            echo '<input type="hidden" class="wtd_cat_excluded" name="wtd_exclude_cats[]" value="' . $ex . '"/>';
                    }
                    ?>
                    <input type="text" name="wtd_search_term" class="wtd_search_input" autocomplete="off" placeholder="Search"/>
                    <select name="wtd_search_resort" class="wtd_search_resort">
                        <option value="0">All Resorts</option>
                        <?php
                        foreach ($resorts_available as $key => $row)
                            echo '<option value="'.$row->term_id.'">'.$row->name.'</option>';
                        ?>
                    </select>
                    <div class="wtd_search_suggestions">
                        <a href="javascript:void(0);" class="wtd_all_results">See All Results&raquo;</a>
                    </div>
                    <button type="submit" class="wtd_search_button"><img src="<?php echo WTD_PLUGIN_URL.'assets/img/search_icon.png';?>"/><span>GO</span></button>
        ...
        public function quick_search(){
            ...
            if ($search_query->found_posts){
                foreach ($search_query->posts as $post){
                    $cats = wp_get_post_terms($post->ID,'wtd_category');
                    foreach ($cats as $c){
                        if (!empty($c->parent)) {
                            if(empty($categories[$c->parent]))
                                $categories[$c->parent] = array( $post->id => $post );
                            else
                                $categories[$c->parent][$post->id] = $post;
                        }else{
                            if(empty($categories[$c->parent]))
                                $categories[$c->term_id] = array( $post->id => $post );
                            else
                                $categories[$c->term_id][$post->id] = $post;
                        }
                        break;
                    }
                ...
                $terms = get_terms('wtd_category',array('include' => array_keys($categories)));
                ob_start();
                foreach ($terms as $category): ?>
                    <div class="wtd_suggestion_category">
                        <div class="wtd_suggestion_item wtd_suggestion_title">
                            <?php echo $category->name;?>
                        </div>
                        <?php foreach ($categories[$category->term_id] as $post): ?>
                            <div class="wtd_suggestion_item">
                                <a href="<?php echo get_permalink($post->ID);?>"><?php echo str_replace($search,'<strong>'.$search.'</strong>',$post->post_title);?></a>
                            </div>
                        <?php endforeach; ?>
                    </div>
                <?php endforeach;
                ?>
                <button type="submit" href="javascript:void(0);" class="wtd_all_results">See All Results&raquo;</button>
                <?php
        ...
        public function wp_footer(){?>
            <link rel="stylesheet" href="<?php echo WTD_PLUGIN_URL.'assets/css/wtd_search_page.css?wtd_version='.WTD_VERSION; ?>"/>
            <script>
                var wtd_searching;
                jQuery('.wtd_search_input').keyup(function(){
                    var val = jQuery(this).val();
                    var input = jQuery(this);
                    input.parent().find('.wtd_search_suggestions').first().hide();
                    if(val.length >= 3){
                        if(typeof wtd_searching !== "undefined")
                            wtd_searching.abort();
                        var res_id = parseInt(input.parent().find('.wtd_search_resort').first().val());
                        var search_data = {
                            action: 'wtd_quick_search',
                            search: val
                        };
                        if(res_id)
                            search_data.res_id = res_id;

                        var excluded = [];
                        input.parent().find('.wtd_cat_excluded').each(function(index, value){
                            excluded[index] = jQuery(this).val();
                        });
                        if(excluded.length > 0)
                            search_data.excluded = excluded;

                        wtd_searching = jQuery.ajax({
                            url : wtd_ajax_url,
                            data:search_data,
                            type: 'post',
                            dataType: 'json',
                            success:function(data){
                                if (data.status)
                                    input.parent().find('.wtd_search_suggestions').first().html(data.html).show();
                            }
                        });
                    }else{
                        if(typeof wtd_searching !== "undefined")
                            wtd_searching.abort();
                        input.next('.wtd_search_suggestions').hide();
                    }
                });
                jQuery('.wtd_search_input').focusout(function(){
                    var input = jQuery(this);
                    setTimeout(function(){
                        input.parent().find('.wtd_search_suggestions').first().hide();
                    }, 200);
                });
                jQuery('.wtd_search_input').focus(function(){
                    var val = jQuery(this).val();
                    var input = jQuery(this);
                    var suggestions = input.parent().find('.wtd_search_suggestions').first();
                    if(val.length >= 3 && suggestions.find('.wtd_suggestion_category').length)
                        suggestions.show();
                });
            </script><?php
        }
        ...
        public function page_content($content){
            ...
            $wtd_pages = get_option('wtd_pages');
            if(!empty($wtd_pages['search_page']))
                if (in_array($post->ID,$wtd_pages['search_page'])) {
        ...
        public function results($content){
            ...
            if(!empty($_REQUEST['wtd_search_resort']))
                $res_id = $_REQUEST['wtd_search_resort'];

            if(empty($_REQUEST['wtd_search_term']))
                return '<script>window.location="'.get_home_url().'";</script>';

            $search_term = $_REQUEST['wtd_search_term'];?>
            <link rel="stylesheet" href="<?php echo WTD_PLUGIN_URL.'assets/css/wtd_activities_page.css?wtd_version='.WTD_VERSION; ?>" media="screen"/>

            <div id="wtd_listing_sc_container"><?php
	            ...
	            $exclude = array();
                if(!empty($_REQUEST['wtd_exclude_cats']))
                    $exclude = $_REQUEST['wtd_exclude_cats'];
                $exclude = array_merge($exclude,$wtd_excluded_categories);
	            $search_args['tax_query'][] = array(
	                'taxonomy' => 'wtd_category',
	                'field' => 'id',
	            ...
	            if ($query->max_num_pages > 1){
	                $pagination =paginate_links( array(
	                    ...
	                    'add_args' => array(
	                        'wtd_search_term' => $_REQUEST['wtd_search_term'],
	                        'wtd_search_resort' => $_REQUEST['wtd_search_resort'],
	                        'wtd_exclude_cats' => $_REQUEST['wtd_exclude_cats']
	                    )
	                ));
	            }
	            if($query->found_posts):
	                ...
	                foreach ($query->posts as $activity):
	                    ...
	                    $metas = $wpdb->get_results($wpdb->prepare('SELECT * FROM wp_postmeta WHERE post_id = %s',$activity->ID));
	                    ...
	                    <div class="wtd_listing_container <?php echo $masonry_item;?>">
	                        <div class="wtd_listing_sc_listing_list">
	                            <div class="wtd_listing_sc_top_content clearfix">
	                                ...
	                                if (!empty($title) && !empty($vendor->post_title))
	                                    $title .= ', ';
	                                if (!empty($vendor->post_title))
	                                ...
	                                <div class="wtd_listing_sc_imageArea">
	                                    <a href="<?php echo $listing_url;?>">
	                                        <img src="<?php echo wtd_get_image_url($listing_thm);?>" alt="Vendor Image"/>
	                                    </a>
	                                </div>
	                ...
	                No items matching your criteria.<?php
	            endif;?>
            </div><?php
    ...
    $wtd_search_page = new wtd_search_page;
}?>